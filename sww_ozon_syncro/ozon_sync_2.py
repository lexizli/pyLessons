...
write_to_csv(sww_ordered_sum, 'sww_ordered_sum.csv', 'product_code;ordered\n')
...
r_file.close()
ozon_stock.sort()
write_to_csv(ozon_stock, 'ozon_stock.csv', 'product_code;stock\n')
ozon_ordered.sort()
...
if ozon_no_orders:
    ozon_ordered.append("" + ';' + "")
write_to_csv(ozon_ordered, 'ozon_ordered.csv', 'product_code;ordered\n')
...
sww_available = []
ordered_ozon_alert = []     # for product codes in ozon if ozon_stock > 0
...
current_ozon_stock = ozon_stock.__next__()
current_ozon_stock_product_code = current_ozon_stock['product_code']  # product code from first string
...
for sww_row in s_o_clear:
    print("248 - ", sww_row['product_code'])
    if sww_row['product_code'] > '2001459819968':
        pussy = 1
        pass
    # минусуем из доступных остатков склада SWW товары, заказанные на sww.com.ru
    if not sww_no_orders:
        if sww_row['product_code'] > current_sww_product_code and more_iteration:
    ...
    #             ordered_ozon_toomuch.append(str(current_sww_product_code) + ';' + str(ordered_current_ozon['ordered']))
    #             try:
    #                 ordered_current_ozon = ozon_ordered.__next__()
...
for strin in ordered_sww_toomuch:
    print("ordered_sww_toomuch — " + strin)

# for strin in ordered_ozon_toomuch:
#     print(strin)